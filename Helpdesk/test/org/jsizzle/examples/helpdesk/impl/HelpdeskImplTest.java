...
{
    private final HelpdeskImpl.Instrumented helpdesk = new HelpdeskImpl().new Instrumented(); 
    
    /**
     * When a customer calls in to the support hotline, an analyst will create
     * an open issue in the system for that customer. The issue is initially
     * assigned to the analyst that created it.
     */
    @Test
    public void addIssue()
    {
        helpdesk.addIssue(helpdesk.addCustomer("Fred"), helpdesk.addAnalyst("Barney"));
    }
     ...
     */
    @Test
    public void reassignIssue()
    {
        final Issue issue = helpdesk.addIssue(helpdesk.addCustomer("Fred"), helpdesk.addAnalyst("Barney"));
        issue.setAnalyst(helpdesk.addAnalyst("Wilma"));
    }

    /**
     * A number of different notes can be added to the issue as the analyst
     * works through it.
     */
    @Test
    public void addDifferentNotes()
    {
     ...
     */
    @Test
    public void crossReferenceIssue()
    {
     ...
     * <ul>
     * <li>A text note allows the analyst to enter free text only.</li>
     * <li>An attachment note allows the analyst to attach any file.</li>
     * <li>A resolution note allows the analyst to record a reason of “user
     * error”, “bug” or “enhancement” against the issue.</li>
     * </ul>
     */
    @Test
    public void addAllNoteTypes()
    {
        final Person barney = helpdesk.addAnalyst("Barney");
        final Issue issue = helpdesk.addIssue(helpdesk.addCustomer("Fred"), barney);
        issue.addNote(barney, "Note1");
    ...
    public void cannotAddNoteAfterResolution()
    {
        final Person barney = helpdesk.addAnalyst("Barney");
        final Issue issue = helpdesk.addIssue(helpdesk.addCustomer("Fred"), barney);
        issue.addNote(barney, Issue.Resolution.BUG);
     ...
     */
    @Test
    public void closeResolvedIssue()
    {
        final Person barney = helpdesk.addAnalyst("Barney");
        final Issue issue = helpdesk.addIssue(helpdesk.addCustomer("Fred"), barney);
        issue.addNote(barney, Issue.Resolution.BUG);
    ...
    @Test(expected = IllegalStateException.class)
    public void cannotCloseUnresolvedIssue()
    {
        final Person barney = helpdesk.addAnalyst("Barney");
        final Issue issue = helpdesk.addIssue(helpdesk.addCustomer("Fred"), barney);
        issue.close();
     ...
     */
    @Test
    public void reportOpenAnalystIssues()
    {
        final Person barney = helpdesk.addAnalyst("Barney");
        final Person fred = helpdesk.addCustomer("Fred");
